...
Deno.serve(async (req) => {
  try {
    ...
    const now = new Date();
    // Se não tiver data de expiração, consideramos válido ou expirado? Vamos considerar inválido se não tiver data.
    if (!request.rating_link_expires_at) {
       return Response.json({ error: 'Link sem data de expiração definida' }, { status: 410 });
    }

    const expiresAt = new Date(request.rating_link_expires_at);
    if (expiresAt < now) {
