...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { serviceRequestIds, groupingType, recipientEmail, sendEmail, period_start, period_end } = body;
    ...
    const suppliers = await base44.entities.Supplier.filter({ id: user.supplier_id });
    ...
    const serviceRequests = await Promise.all(
      serviceRequestIds.map(id => 
        base44.entities.ServiceRequest.filter({ id }).then(r => r[0])
      )
    );

    // Os que não foram encontrados, tenta buscar como SupplierOwnBooking
    const missingIds = serviceRequestIds.filter((id, index) => !serviceRequests[index]);
    
    let ownBookings = [];
    if (missingIds.length > 0) {
        ownBookings = await Promise.all(
          missingIds.map(id => 
            base44.entities.SupplierOwnBooking.filter({ id }).then(r => r[0])
          )
        );
    }
    ...
    const [clients, ownClients, users] = await Promise.all([
      Promise.all(clientIds.map(id => base44.entities.Client.filter({ id }).then(r => r[0]))),
      Promise.all(ownClientIds.map(id => base44.entities.SupplierOwnClient.filter({ id }).then(r => r[0]))),
      Promise.all([...userIds, ...billingUserIds].map(id => 
        base44.asServiceRole.entities.User.filter({ id }).then(r => r[0])
      ))
    ]);
    ...
    if (period_start && period_end) {
      doc.setFontSize(9);
      const periodText = `Periodo: ${new Date(period_start).toLocaleDateString('pt-BR')} a ${new Date(period_end).toLocaleDateString('pt-BR')}`;
    ...
    for (const [groupName, groupRequests] of Object.entries(groupedData)) {
      ...
      groupRequests.forEach((req, idx) => {
        ...
        doc.text(`${new Date(req.date).toLocaleDateString('pt-BR')} ${req.time || ''}`.trim(), margin + 20, yPos + 4);
    ...
    doc.text(formatCurrency(totalGeral), margin + 220, yPos + 5.5);
    ...
    if (sendEmail && recipientEmail) {
      ...
      await base44.integrations.Core.SendEmail({
        ...
        body: `
          ...
          <p>Total de viagens: ${validRequests.length}</p>
          <p>Valor total: ${formatCurrency(totalGeral)}</p>
      ...
      return Response.json({
        ...
        tripCount: validRequests.length,
