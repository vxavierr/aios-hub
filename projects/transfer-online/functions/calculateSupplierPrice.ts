  ...
  try {
    ...
    try {
      ...
      if (service_type === 'one_way') {
        ...
        const distResult = await base44.asServiceRole.functions.invoke('calculateDistance', {
          origin: baseAddress,
          waypoints: [origin, destination],
          destination: baseAddress
        });
        
        console.log('[calculateSupplierPrice] Resultado distância (one_way):', distResult.data);
        
        if (distResult.data) {
          totalDistanceKm = parseFloat(distResult.data.distance_km);
          durationMinutes = distResult.data.duration_minutes;
          distanceData = distResult.data;
        }
      } else if (service_type === 'round_trip') {
        // ROUND_TRIP: Calcular IDA e VOLTA separadamente
        console.log('[calculateSupplierPrice] Calculando IDA e VOLTA separadamente...');
        
        // IDA: base -> origem -> destino -> base
        const outboundResult = await base44.asServiceRole.functions.invoke('calculateDistance', {
          origin: baseAddress,
          waypoints: [origin, destination],
          destination: baseAddress
        });
        
        console.log('[calculateSupplierPrice] Resultado IDA:', outboundResult.data);
        
        // VOLTA: base -> destino (origem da volta) -> origem (destino da volta) -> base
        const returnResult = await base44.asServiceRole.functions.invoke('calculateDistance', {
          origin: baseAddress,
          waypoints: [destination, origin],
          destination: baseAddress
        });
        
        console.log('[calculateSupplierPrice] Resultado VOLTA:', returnResult.data);
        ...
        if (outboundResult.data && returnResult.data) {
          ...
          durationMinutes = outboundResult.data.duration_minutes + returnResult.data.duration_minutes;
          ...
          distanceData = {
            ...
            outbound_duration_minutes: outboundResult.data.duration_minutes,
            return_duration_minutes: returnResult.data.duration_minutes
          };
        }
      } else if (service_type === 'hourly') {
        // HOURLY: Base -> Origem -> Paradas -> Destino (se houver) -> Base
        const waypoints = [origin];
        
        if (additional_stops && additional_stops.length > 0) {
          waypoints.push(...additional_stops);
        }
        
        // Se houver destino, adicionar
        ...
        console.log('[calculateSupplierPrice] Resultado distância (hourly):', distResult.data);
