...
Deno.serve(async (req) => {
  try {
    ...
    try {
      ...
      // Allow continuing without user if public pricing is enabled
    }

    // Check if public pricing is enabled
    const allConfigs = await base44.asServiceRole.entities.AppConfig.filter({}); // Fetch all relevant configs at once
    const getConfig = (key) => allConfigs.find(c => c.config_key === key)?.config_value;
    ...
    // If no user AND public pricing disabled (or out of date range) AND not internal call, return Unauthorized
    if (!user && !canViewPricesWithoutLogin && !is_internal_call) {
    ...
    if (service_type === 'one_way') {
      ...
      let distanceResult;
      try {
        distanceResult = await base44.asServiceRole.functions.invoke('calculateDistance', {
        ...
        const distMsg = distErr.response?.data?.error || distErr.message;
        return Response.json({ error: `Falha ao calcular rota: ${distMsg}` }, { status: 500 });
      }

      if (!distanceResult.data || !distanceResult.data.distance_km) {
        return Response.json(
          { error: 'Erro ao calcular distância: Resposta inválida' },
          { status: 500 }
        );
      }

      const supplierTotalDistanceKm = distanceResult.data.distance_km;
      const supplierDurationMinutes = distanceResult.data.duration_minutes;
    ...
    // Cálculo para ROUND_TRIP
      ...
      try {
        [leg1Result, leg2Result] = await Promise.all([
          base44.asServiceRole.functions.invoke('calculateDistance', {
            origin: supplierBaseAddress,
            waypoints: [origin, destination],
            destination: supplierBaseAddress,
            is_internal_call: true
         ...
         const distMsg = distErr.response?.data?.error || distErr.message;
         return Response.json({ error: `Falha ao calcular rota (RT): ${distMsg}` }, { status: 500 });
      }

      if (!leg1Result.data?.distance_km || !leg2Result.data?.distance_km) {
        return Response.json({ error: 'Erro ao calcular distância das pernas da viagem' }, { status: 500 });
      }

      const dist1 = parseFloat(leg1Result.data.distance_km);
      const dist2 = parseFloat(leg2Result.data.distance_km);
      const dur1 = leg1Result.data.duration_minutes;
      const dur2 = leg2Result.data.duration_minutes;
