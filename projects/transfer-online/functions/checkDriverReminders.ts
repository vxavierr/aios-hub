...
Deno.serve(async (req) => {
  try {
    ...
    for (const trip of allTrips) {
        ...
        // Janela de envio: 45 a 75 min antes (aprox 1h)
        if (timeDiffMinutes >= 45 && timeDiffMinutes <= 75) {
            ...
            // 2. Tentar Email
            if (driverEmail) {
                const emailSubject = `🔔 Lembrete de Viagem - ${trip.display_id}`;
                const emailBody = `
            ...
            // Se enviou por qualquer canal, atualiza a flag
            if (messageSent) {
                const Entity = base44.asServiceRole.entities[trip.type];
                ...
                try {
                    await base44.asServiceRole.entities.TripHistory.create({
                        ...
                        details: {
                            sent_via_whatsapp: !!driverPhone,
                            sent_via_email: !!driverEmail,
