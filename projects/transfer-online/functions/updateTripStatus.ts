...
Deno.serve(async (req) => {
  try {
    ...
    const updateData = {
      ...
      command_history: [
        ...(serviceRequest.command_history || []),
        commandEntry
      ]
    };

    // Lógica de Paradas Adicionais (Cronômetro) e Paradas Planejadas
    let currentStops = [...(serviceRequest.additional_stops || [])];
    let stopsUpdated = false;
    let plannedStopsUpdated = false;
    let currentPlannedStops = [...(serviceRequest.planned_stops || [])];
    ...
    const sideEffectsPromise = (async () => {
            ...
            if (finalLat && finalLon && finalLat !== 0 && finalLon !== 0) {
                ...
                if (calcOrigin && calcDestination) {
                    try {
                        ...
                        if (etaResponse.data.success) etaMinutes = etaResponse.data.eta_minutes;
                    } catch (e) { console.error('ETA GPS Error', e); }
                }
            }
            
            // ETA Fallback
            if (!etaMinutes && ['passageiro_embarcou', 'a_caminho_destino'].includes(newStatus) && serviceRequest.origin && serviceRequest.destination) {
                 try {
                    const etaResponse = await base44.functions.invoke('calculateETA', {
        ...
        // 5. Driver Arrival Notification
        if (newStatus === 'chegou_origem') {
             try {
                base44.functions.invoke('notifyDriverArrival', {
                    tripId: serviceRequestId,
                    tripType: isOwnBooking ? 'SupplierOwnBooking' : (isEventTrip ? 'EventTrip' : (isDirectBooking ? 'Booking' : 'ServiceRequest')),
                    overrideDriverName: serviceRequest.driver_name
                }); // Fire and forget inside sideEffectsPromise (which is awaited but we don't await this inside)
            } catch (e) { console.error('Arrival Notification Error', e); }
        }
