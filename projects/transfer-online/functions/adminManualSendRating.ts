...
Deno.serve(async (req) => {
  try {
    ...
    try {
      const response = await base44.asServiceRole.functions.invoke('generateAndSendRatingLink', {
        serviceRequestId: serviceRequestId,
        recipientEmail: null // Deixar a função buscar o email/telefone automaticamente
      });
      
      if (response.status && response.status >= 400) {
         throw new Error(response.data?.error || `Erro na função remota (${response.status})`);
      }
      ...
      await base44.asServiceRole.entities.TripHistory.create({
          ...
          details: {
              reponse_from_generateAndSendRatingLink: response.data,
