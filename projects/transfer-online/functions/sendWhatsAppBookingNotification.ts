...
Deno.serve(async (req) => {
  try {
    ...
    if (!apiUrl || !token || !instanceId) {
      console.error("Credenciais do WhatsApp não configuradas completely");
      return Response.json(
        { success: false, error: 'Configuração do WhatsApp incompleta (Secrets)' },
        { status: 500 }
      );
    }

    // Determinar número do destinatário
    let phoneNumber = '';
    ...
    if (recipientType === 'admin') {
      ...
      phoneNumber = bookingData.customer_phone.replace(/\D/g, '');
    } else {
      return Response.json(
    ...
    // Se for cliente e tiver telefone, verifica se tem + no original
    const originalPhone = recipientType === 'customer' ? bookingData.customer_phone : null;
    
    // Se tem original com +, respeita. Se não tem +, e não começa com 55, adiciona.
    // Para admin (originalPhone null), mantém comportamento padrão (adiciona 55 se curto/sem prefixo)
    if (originalPhone && originalPhone.includes('+')) {
        // Internacional explícito: não força 55, usa o que tiver (o replace já limpou os chars)
