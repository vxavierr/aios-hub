...
Deno.serve(async (req) => {
  try {
    ...
    if (serviceRequest.event_casual_driver_id && isEventTrip) {
        try {
            const casualDriver = await base44.entities.EventCasualDriver.get(serviceRequest.event_casual_driver_id);
            if (casualDriver) {
    ...
    if (!driverAccessToken) {
      ...
      if (isEventTrip) await base44.entities.EventTrip.update(serviceRequestId, updateData);
      else if (isOwnBooking) await base44.entities.SupplierOwnBooking.update(serviceRequestId, updateData);
      else if (isDirectBooking) await base44.entities.Booking.update(serviceRequestId, updateData);
      else await base44.entities.ServiceRequest.update(serviceRequestId, updateData);
    }
