...
Deno.serve(async (req) => {
  try {
    ...
    const [client, vehicle] = await Promise.all([
      base44.asServiceRole.entities.Client.get(client_id).catch(e => { console.error('Erro buscar client:', e); return null; }),
      base44.asServiceRole.entities.SupplierVehicleType.get(chosen_vehicle_type_id).catch(e => { console.error('Erro buscar veiculo:', e); return null; })
    ]);
    ...
    // === VERIFICAÇÃO DE DUPLICIDADE (Non-blocking check se possível ou rápida) ===
    let duplicateWarning = null;
    // Vamos manter, mas com log de tempo
    const startCheck = Date.now();
    try {
      // console.log('[submitServiceRequest] Verificando duplicidade...');
      const checkResponse = await base44.asServiceRole.functions.invoke('checkDuplicateTrips', {
        passenger_user_id: passenger_user_id || undefined,
        passenger_email: passenger_email || undefined,
        date: date,
        time: time
      });

      if (checkResponse.data?.has_duplicate) {
        console.warn('[submitServiceRequest] ⚠️ Duplicidade detectada (permitindo seguir):', checkResponse.data.message);
        duplicateWarning = checkResponse.data.message;
      }
    } catch (dupError) {
      console.error('[submitServiceRequest] Erro ao verificar duplicidade (ignorando):', dupError);
    }
    // console.log(`[submitServiceRequest] Verificação duplicidade levou ${Date.now() - startCheck}ms`);
