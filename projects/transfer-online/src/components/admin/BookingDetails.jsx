...
export default function BookingDetails({ booking, open, onClose, onManageDriver }) {
  ...
  const handleCopySummary = () => {
    ...
    let info = `📋 *Resumo da Viagem - ${booking.booking_number || ''}*\n\n`;
    info += `📅 Data: ${format(parseLocalDate(booking.date), "dd/MM/yyyy", { locale: ptBR })} às ${booking.time}\n`;
    info += `📍 Origem: ${booking.origin}\n`;
    info += `🏁 Destino: ${booking.destination}\n`;

    if (booking.origin_flight_number) {
        info += `✈️ Voo Origem: ${booking.origin_flight_number}\n`;
    }
    if (booking.destination_flight_number) {
        info += `✈️ Voo Destino: ${booking.destination_flight_number}\n`;
    }

    info += `👥 Passageiro: ${booking.customer_name} (${booking.passengers} pax)\n`;
    if (booking.customer_phone) {
        info += `📱 Tel Passageiro: ${booking.customer_phone}\n`;
    }

    info += `\n🚘 *Motorista e Veículo*\n`;
    if (booking.driver_name) {
        info += `👤 Motorista: ${booking.driver_name} ${booking.driver_phone || ''}\n`;
        info += `🚙 Veículo: ${booking.vehicle_model || ''} - ${booking.vehicle_plate || ''}\n`;
    } else {
        info += `⚠️ Motorista não atribuído.\n`;
    }
      ...
      <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
        ...
        <div className="space-y-6">
          ...
          <div className="flex justify-between mt-4">
            <Button onClick={handleCopySummary} variant="outline" className="text-blue-600 border-blue-200 hover:bg-blue-50">
