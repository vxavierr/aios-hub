...
export default function QuoteDialog({ open, onOpenChange, onSuccess }) {
  ...
  const handleAddManualItem = async () => {
    ...
    const newItemPrice = parseFloat(manualItem.price);
    const newItem = {
    ...
    setTripData(prev => ({
      ...prev,
      selected_additional_items: [...(prev.selected_additional_items || []), newItem]
    }));
  ...
  const handleRemoveAdditionalItem = (index) => {
    setTripData(prev => {
      const newItems = [...(prev.selected_additional_items || [])];
      newItems.splice(index, 1);
      return { ...prev, selected_additional_items: newItems };
    });
  };
          ...
          <div className="space-y-4">
            ...
            {step === 2 && quoteFormat !== 'agency' && (
              <div className="space-y-4">
                ...
                <div className="space-y-3 border rounded-lg p-4 bg-gray-50">
                  ...
                  {tripData.selected_additional_items?.length > 0 && (
                    <div className="space-y-2 mb-4 bg-white p-2 rounded border">
                      {tripData.selected_additional_items.map((item, idx) => (
                        <div key={idx} className="flex justify-between items-center text-sm p-1 hover:bg-gray-50 rounded">
                          ...
                          <div className="flex items-center gap-3">
                            ...
                            <Button size="icon" variant="ghost" onClick={() => handleRemoveAdditionalItem(idx)} className="h-6 w-6 text-red-500">
                      ...
                      <div className="text-right text-sm font-bold text-gray-700 pt-2 border-t">
                        Total: {formatPrice(tripData.selected_additional_items.reduce((acc, item) => acc + (item.price * item.quantity), 0))}
                      </div>
                    </div>
                  )}
            ...
            {step === 2 && quoteFormat === 'agency' && (
              <div className="space-y-6">
                ...
                <div className="space-y-3 border rounded-lg p-4 bg-gray-50">
                  <Label className="font-bold block">Itens Adicionais (Global)</Label>
