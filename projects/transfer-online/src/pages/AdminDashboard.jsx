...
import { Calendar, DollarSign, Users, CheckCircle, Search, Filter, XCircle, Clock, Loader2, AlertCircle, Navigation, BellRing, Settings } from 'lucide-react';
...
export default function AdminDashboard() {
  ...
  const allTrips = React.useMemo(() => {
    ...
    return [...normalizedBookings, ...normalizedRequests, ...normalizedSupplierBookings].sort((a, b) => {
      return new Date(b.created_date) - new Date(a.created_date);
    });
  }, [bookings, serviceRequests, supplierOwnBookings, clients, suppliers, supplierClients]);
  ...
  const updateStatusMutation = useMutation({
    ...
    onMutate: async ({ trip, newStatus }) => {
      ...
      return { previousBookings, previousRequests, previousSupplierBookings };
    },
  ...
  const stats = React.useMemo(() => {
    // Contagens Totais
    const totalTrips = bookings.length + serviceRequests.length + supplierOwnBookings.length;
  ...
  }, [bookings, serviceRequests, supplierOwnBookings]);
