...
export default function MinhasSolicitacoesFornecedor() {
  ...
  const paginatedOtherRequests = otherRequests.slice((currentPage - 1) * itemsPerPage, currentPage * itemsPerPage);
  ...
  return (
          ...
          <TabsContent value="list" className="space-y-6">
             <Suspense fallback={<SectionLoader />}>
                <Tabs defaultValue="critical" className="space-y-4">
                  <TabsList className="grid w-full grid-cols-2 md:grid-cols-4 gap-2 h-auto p-1 bg-gray-100/50">
                    <TabsTrigger value="critical" className="data-[state=active]:bg-red-600 data-[state=active]:text-white h-full py-2 whitespace-normal text-xs sm:text-sm">
                        <span className="flex items-center gap-1 justify-center flex-wrap">
                            <span>🚨 Ação</span>
                            <Badge variant="secondary" className="bg-white/20 text-current hover:bg-white/30 border-0 px-1 py-0 h-5 min-w-[1.25rem]">{criticalRequests.length}</Badge>
                        </span>
                    </TabsTrigger>
                    <TabsTrigger value="today" className="data-[state=active]:bg-blue-600 data-[state=active]:text-white h-full py-2 whitespace-normal text-xs sm:text-sm">
                        <span className="flex items-center gap-1 justify-center flex-wrap">
                            <span>🔥 Hoje</span>
                            <Badge variant="secondary" className="bg-white/20 text-current hover:bg-white/30 border-0 px-1 py-0 h-5 min-w-[1.25rem]">{todayRequestsList.length}</Badge>
                        </span>
                    </TabsTrigger>
                    <TabsTrigger value="in_progress" className="data-[state=active]:bg-purple-600 data-[state=active]:text-white h-full py-2 whitespace-normal text-xs sm:text-sm">
                        <span className="flex items-center gap-1 justify-center flex-wrap">
                            <span>🚗 Andamento</span>
                            <Badge variant="secondary" className="bg-white/20 text-current hover:bg-white/30 border-0 px-1 py-0 h-5 min-w-[1.25rem]">{inProgressRequestsList.length}</Badge>
                        </span>
                    </TabsTrigger>
                    <TabsTrigger value="other" className="h-full py-2 whitespace-normal text-xs sm:text-sm">
                  ...
                  <TabsContent value="today">
                    <Card className="border-2 border-blue-400">
                      <CardContent className="p-0">
                        <RequestTable 
    requests={todayRequestsList} 
    onViewDetails={handleViewDetails} 
    onManageDriver={handleOpenDriverInfoDialog} 
    formatPrice={formatPrice} 
    showDriverAction showTripStatus 
    parseLocalDate={parseLocalDate}
    supplierVehicleTypes={supplierVehicleTypes}
    selectedTrips={selectedTrips}
    onSelectionChange={setSelectedTrips}
    ...
    />
                  ...
                  <TabsContent value="in_progress">
                    <Card><CardContent className="p-0"><RequestTable 
    requests={inProgressRequestsList} 
    onViewDetails={handleViewDetails} 
    onManageDriver={handleOpenDriverInfoDialog} 
    formatPrice={formatPrice} 
    showDriverAction showTripStatus
    ...
    /></CardContent></Card>
                  ...
                  <TabsContent value="other">
                    <Card>
                      <CardContent className="p-0">
                        <RequestTable 
    requests={paginatedOtherRequests} 
    onViewDetails={handleViewDetails} 
    onManageDriver={handleOpenDriverInfoDialog} 
    formatPrice={formatPrice} 
    showTripStatus 
