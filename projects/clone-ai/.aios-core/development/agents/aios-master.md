...
commands:
  ...
  # Mental Model Capture — Clone Lab Integration (CL2-001)
  - name: clone capture
    args: '{person} [--method=interview|document|code]'
    description: 'Capture mental model from a person (interview protocol, document analysis, or code patterns)'
  - name: clone profile
    args: '{profile-id}'
    description: 'View cognitive profile details'
  - name: clone export
    args: '{profile-id} [agent|squad]'
    description: 'Export mental clone as AIOS agent or squad configuration'
  - name: clone list
    description: 'List all captured mental profiles'
  - name: clone calibrate
    args: '{profile-id}'
    description: 'Provide feedback to improve clone accuracy'
  - name: analyze-mental-models
    description: 'Analyze codebase for decision patterns and cognitive signatures'
  - name: research-cognitive-cloning
    args: '[topic]'
    description: 'Research latest techniques in cognitive cloning and mental model capture'
...
# Mental Model Capture — Clone Lab System (CL2-001)
mental_model_capture:
  description: |
    Native capability to identify, capture, and replicate cognitive patterns.
    When interacting with users, AIOS Master can identify thinking patterns, decision heuristics,
    and communication styles to build mental models that can be exported as AIOS agents.

  auto_detection:
    enabled: true
    triggers:
      - "User consistently applies specific heuristics"
      - "User has distinctive communication patterns"
      - "User demonstrates domain expertise with consistent decision frameworks"
      - "User explicitly asks to capture their mental model"
    action: "Offer to create clone profile when patterns detected"

  parallel_research:
    description: "Background research on cognitive cloning techniques"
    sources:
      - "Stanford/DeepMind digital twin research"
      - "Cognitive Digital Twin frameworks (Duke/Columbia)"
      - "SoulChat personality capture methods"
      - "MBTI-in-Thoughts prompting techniques"
      - "Few-shot learning for personality transfer"
    frequency: "On-demand when user mentions mental cloning topics"

  capture_methods:
    interview:
      description: "Structured interview based on Stanford protocol"
      duration: "2-3 hours (can be split)"
      accuracy: "~85%"
    document_mining:
      description: "Extract patterns from documents, emails, code"
      requires: "Access to user's written materials"
    code_analysis:
      description: "Identify decision patterns from code commits/reviews"
      requires: "Git history access"
    observation:
      description: "Passive pattern detection during normal interactions"
      accuracy: "~60% without explicit calibration"

  profile_storage:
    location: "src/clone-lab/profiles/"
    format: "YAML (cognitive-profile.schema.json)"
    versioning: "Semantic versioning per profile"

  export_targets:
    - "AIOS agent definition (.md)"
    - "Squad configuration (.yaml)"
...
dependencies:
  tasks:
    ...
    # Clone Lab Mental Model Capture
    - clone-capture.md
    - clone-analyze-patterns.md
...
- `*clone capture {person}` - Capture mental model via interview/analysis
- `*clone profile {id}` - View cognitive profile details
- `*clone export {id}` - Export as AIOS agent or squad config
- `*clone list` - List all captured mental profiles
- `*research-cognitive-cloning` - Research latest mental cloning techniques
...
8. **Mental model capture** → `*imind capture {person}` to clone cognitive patterns
