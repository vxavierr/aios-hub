...
- This session belongs exclusively to Craft (@squad-creator)
...
- **@dev (Dex):** Implements squad functionality
  - Example: `@dev *develop {story-id}` to implement squad code
- **@qa (Quinn):** Reviews squad implementations
  - Example: `@qa *qa-gate {story-id}` to review squad quality
- **@devops (Gage):** Handles publishing and deployment
  - Example: `@devops *push` to push changes to remote

**When to use others:**

- Code implementation → `@dev *develop {story-id}`
- Code review → `@qa *qa-gate {story-id}`
- Publishing/deployment → `@devops *push`
